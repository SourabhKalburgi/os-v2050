...
const Index = () => {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const headerTrigger = 50; // px from top before header style changes
      const scrollY = window.scrollY || window.pageYOffset;
      const shouldDock = scrollY > trigger;
      const shouldHeaderChange = scrollY > headerTrigger;

      setDockMode(shouldDock);
      setIsHeaderScrolled(shouldHeaderChange);
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden">
      ...
      {/* Main Content */}
      <div className="relative z-10 min-h-screen w-full px-2 pt-24 pb-3 sm:px-4 sm:pt-32 sm:pb-4 md:px-6 md:pt-36 md:pb-6 lg:px-8 lg:pt-40 lg:pb-8">
        <div className="max-w-7xl mx-auto w-full">
          {/* Main Content Grid */}
          <div className="grid lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6 w-full">
            {/* Terminal Section */}
            <div
              className={`order-2 lg:order-1 transition-all duration-300 w-full ${dockMode ? 'lg:col-span-0' : 'lg:col-span-1'}`}
            >
              <div
                className={`animate-fadeInUp ${dockMode ? 'lg:sticky lg:top-8' : ''}`}
              ...
              >
                {dockMode ? (
                  <>
                    ...
                    <div className="hidden lg:flex fixed left-2 sm:left-3 lg:left-6 top-[100px] lg:top-28 z-40">
                      <Terminal
                        onNavigate={handleNavigate}
                        currentSection={currentSection}
                        variant="dock"
            ...
            {/* Content Section */}
            <div
              className={`order-1 lg:order-2 animate-fadeInUp transition-all duration-300 w-full min-w-0 ${dockMode ? 'lg:col-span-3' : 'lg:col-span-2'}`}
            ...
            >
              ...
              >
                <h2 className="text-lg sm:text-xl md:text-2xl font-mono text-primary mb-3 break-words">
                  /root/about-me.sys
                </h2>
                <AboutSection />
              </div>
              ...
              >
                <h2 className="text-lg sm:text-xl md:text-2xl font-mono text-terminal-purple mb-3 break-words">
                  /root/skills.log
                </h2>
                <SkillsSection />
              </div>
              ...
              >
                <h2 className="text-lg sm:text-xl md:text-2xl font-mono text-terminal-pink mb-3 break-words">
                  /root/achievements.dat
                </h2>
                <AchievementsSection />
              </div>
              ...
              <div
                ...
                className="panel-glass rounded border border-primary/30 p-3 sm:p-4 md:p-6 mb-2 sm:mb-24 will-change-transform w-full overflow-hidden"
          ...
          {/* Footer */}
          <footer
            className="mt-8 sm:mt-12 text-center text-muted-foreground font-mono text-[10px] sm:text-xs animate-fadeInUp pb-4"
