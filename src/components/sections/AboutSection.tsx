    ...
    <div className="space-y-3 sm:space-y-4 md:space-y-6 animate-fadeInUp w-full">
          ...
          <div className="mt-3 sm:mt-4 md:mt-6 space-y-2 w-full">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 w-full">
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <span className="text-terminal-purple text-[10px] sm:text-xs md:text-sm">STATUS</span>
                <span className="text-primary">{']'}</span>
              </div>
              <span className="text-terminal-cyan text-[10px] sm:text-xs md:text-sm break-words">Active // Available for collaboration</span>
            </div>
            
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 w-full">
              <div className="flex items-center gap-1 flex-shrink-0">
                <span className="text-primary">{'['}</span>
                <span className="text-terminal-purple text-[10px] sm:text-xs md:text-sm">LOCATION</span>
                <span className="text-primary">{']'}</span>
              </div>
              <span className="text-terminal-cyan text-[10px] sm:text-xs md:text-sm break-words">Bangalore // 12.9629° N, 77.5775° E</span>
            </div>
            
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 w-full">
              <div className="flex items-center gap-1 flex-shrink-0">
                <span className="text-primary">{'['}</span>
                <span className="text-terminal-purple text-[10px] sm:text-xs md:text-sm">MISSION</span>
                <span className="text-primary">{']'}</span>
              </div>
              <span className="text-terminal-cyan text-[10px] sm:text-xs md:text-sm break-words">Create digital experiences that make people say, "Wait… you built THIS?"</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4 w-full">
        {[
          ...
          >
            <div className={`text-xl sm:text-2xl md:text-3xl font-bold ${stat.color} text-glow mb-1 break-words`}>{stat.value}</div>
            <div className="text-[9px] sm:text-[10px] md:text-xs text-muted-foreground font-mono break-words leading-tight">{stat.label}</div>
          </div>
        ))}
      </div>
    </div>
